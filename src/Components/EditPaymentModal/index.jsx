...
function EditPaymentModal({ open, setOpen, payment }) {
  const { title, description, due_date, _id } = payment || {};

  const dispatch = useDispatch();
  const { editPaymentLoading, editPaymentData } = useSelector(
    (state) => state.EditPaymentReducer
  );
  ...
  const onSubmit = (obj) => {
    ...
    dispatch(EditPaymentAction(objToSend, toast));
  };
  ...
  useEffect(() => {
    if (editPaymentData) {
      handleClose();
      reset();
      dispatch(EditPaymentEmptyAction());
    }
  }, [editPaymentData]);
            ...
            <Box component={"form"} onSubmit={handleSubmit(onSubmit)}>
              ...
              <Box sx={{ margin: "20px 0" }}>
                {editPaymentLoading ? (
                  ...
                  <ButtonCmp
                    title={Strings.dashboard.editPayment}
                    type="submit"
                  />
                )}
              </Box>
